...
// Device code
__global__ void VecMax(const float* A, float* C, int N)
{
    ...
    while (i < N) {
        temp = max(A[i], temp);
        i += blockDim.x*gridDim.x;  // go to next grid
    }
    ...
    while (ib != 0) {
      if(cacheIndex < ib)
        cache[cacheIndex] = max(cache[cacheIndex],cache[cacheIndex + ib]); 
...
//    printf("The number of blocks per grid:%d\n",blocksPerGrid);
    ...
    VecMax <<< blocksPerGrid, threadsPerBlock, sm >>>(d_A, d_C, N);
